...
type
  JsonEventKind* = enum ## enumeration of all events that may occur when parsing
    ...
    malNil,             ## the value `nil`
    jsonObjectStart,    ## start of an object: the `{` token
    jsonObjectEnd,      ## end of an object: the `}` token
    jsonArrayStart,     ## start of an array: the `[` token
    jsonArrayEnd,       ## end of an array: the `]` token
    sExprStart,         ## start of an sexpr: the `(` token
    sExprEnd            ## end of an sexpr: the `)` token
  ...
  ParserState = enum
    stateEof, stateStart, stateObject, stateArray, stateSExpr, stateExpectArrayComma,
    stateExpectSExprSpace, stateExpectObjectComma, stateExpectColon, stateExpectValue
...
proc parseSymbol(my: var JsonParser) =
  var pos = my.bufpos
...
proc next*(my: var JsonParser) =
  ...
  of stateStart:
    ...
    of tkParLe:
      my.state.add(stateSExpr) # we expect any
      my.kind = sExprStart
    of tkCurlyLe:
  ...
  of stateSExpr:
    case tk
    of tkString, tkSymbol, tkInt, tkTrue, tkFalse, tkNil: #, tkFloat , tkNull
    ...
    of tkBracketLe:
      ...
      my.kind = jsonArrayStart
    of tkParLe:
      my.state.add(stateExpectSExprSpace)
      my.state.add(stateSExpr)
      my.kind = sExprStart
    of tkCurlyLe:
      my.state.add(stateExpectArrayComma)
  ...
  of stateExpectSExprSpace:
    ...
    of tkParRi:
      my.kind = sExprEnd
      discard my.state.pop() # pop stateExpectSExprSpace
      discard my.state.pop() # pop stateSExpr
    else:
      my.kind = jsonError
      my.err = errBracketRiExpected
  ...
  of stateExpectValue:
    ...
    of tkBracketLe:
      ...
      my.kind = jsonArrayStart
    of tkParLe:
      my.state[i] = stateExpectSExprSpace
      my.state.add(stateSExpr)
      my.kind = sExprStart
    of tkCurlyLe:
      my.state[i] = stateExpectObjectComma
