...
## `JObject`, `JArray`, `SExpr`, `JString`, `JInt`, `JFloat`, `JBool` or
...
proc newSExpr*(): JsonNode =
  ## Creates a new `SExpr JsonNode`
  result = JsonNode(kind: SExpr, sexprs: @[])
...
proc getSym*(n: JsonNode, default: string = ""): string =
  ## Retrieves the string value of a `Symbol JsonNode`.
  ##
...
proc getSExprs*(n: JsonNode, default: seq[JsonNode] = @[]): seq[JsonNode] =
  ## Retrieves the array of a `SExpr JsonNode`.
  ##
  ## Returns `default` if `n` is not a `SExpr`, or if `n` is nil.
  if n.isNil or n.kind != SExpr: return default
  else: return n.sexprs

proc add*(father, child: JsonNode) =
  ## Adds `child` to a JArray or SExpr node `father`.
  case father.kind
  of JArray:
    father.elems.add(child)
  of SExpr:
    father.sexprs.add(child)
  else:
    discard
...
proc toJsonImpl(x: NimNode): NimNode =
  ...
  of nnkPar: # sexpr
    if x.len == 0: return newCall(bindSym"newSExpr")
...
proc copy*(p: JsonNode): JsonNode =
  ...
  of SExpr:
    result = newSExpr()
    for i in items(p.sexprs):
      result.sexprs.add(copy(i))
...
iterator sexprs*(node: JsonNode): JsonNode =
  ## Iterator for the items of `node`. `node` has to be a SExpr.
  assert node.kind == SExpr, ": sexprs() can not iterate a JsonNode of kind " & $node.kind
  for i in items(node.sexprs):
    yield i
...
proc parseJson(p: var JsonParser; rawIntegers, rawFloats: bool, depth = 0): JsonNode =
  ...
  of tkSymbol:
    # we capture 'p.a' here, so we need to give it a fresh buffer afterwards:
    when defined(gcArc) or defined(gcOrc) or defined(gcAtomicArc):
      result = JsonNode(kind: Symbol, sym: move p.a)
    else:
      result = JsonNode(kind: Symbol)
  ...
  of tkParLe:
    ...
    while p.tok != tkParRi:
      ...
      discard getTok(p)
    eat(p, tkParRi)
  of tkError, tkCurlyRi, tkBracketRi, tkParRi, tkColon, tkComma, tkSpace, tkEof:
    raiseParseErr(p, "{")
